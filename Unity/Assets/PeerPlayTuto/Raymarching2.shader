    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float4 distanceField(float3 p, float depth) 
			{
				...
				//float3 sphere1Pos = _spheres[0].xyz + float3(8 * sin(_Time.y * speed), 10 * sin(_Time.y * speed), 5 * cos(_Time.y * speed));

				////Scale
				//float PI = 3.14159265f;
				//float sphere1Size = _spheres[0].w + 3 * (0.5 + 0.5 * sin(2 * PI * _Time.y * 0.3 + (p.y - sphere1Pos.y) / 3));

				//Distance
				//float4 ground = float4(_groundColor.rgb, sdPlane(p, float4(0, 1, 0, 0)));
				//float4 sphere = float4(_sphereColor[0].rgb, sdSphere((p - sphere1Pos.xyz), 4));
				...
				for (int i = 0; i < 8; i++)
				{
					...
					//Scale
					float PI = 3.14159265f;
					float sphereSize = 4 + 3 * (0.5 + 0.5 * sin(2 * PI * _Time.y * 0.3 + (p.y - _spheres[i].y) / 3));

					//Distance
					float4 sphereAdd = float4(_sphereColor[i].rgb, sdSphere(p - _spheres[i].xyz, _spheres[i].w));
					float4 smooth = opIS(distanceScene, sphereAdd, _sphereSmooth);
					float4 sphereSmooth = opUS(sphereAdd, smooth, _sphereSmooth);

					result = opUS(result, sphereSmooth, _sphereSmooth);
					//result = opU(result, smooth);
				}
